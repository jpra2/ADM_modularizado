...
lxd2=[]
for i in range(int(len(lxd1)*l1[0]/l2[0])-1):
    lxd2.append(lxd1[int(i*l2[0]/l1[0])])
lxd2.append(lxd1[-1])

lyd2=[]
for i in range(int(len(lyd1)*l1[1]/l2[1])-1):
    lyd2.append(lyd1[int(i*l2[1]/l1[1])])
lyd2.append(lyd1[-1])

lzd2=[]
for i in range(int(len(lzd1)*l1[2]/l2[2])-1):
    lzd2.append(lzd1[int(i*l2[2]/l1[2])])
lzd2.append(lzd1[-1])
...
for i in range(len(lx2)-1):
    ...
    for j in range(len(ly2)-1):
        ...
        for k in range(len(lz2)-1):
            ...
            #    elem_por_D2 = get_box(M1.all_volumes, centroids, box_dual2, False)
...
for i in range(len(lxd1)-1):
    ...
    for j in range(len(lyd1)-1):
        ...
        for k in range(len(lzd1)-1):
            ...
            vols=get_box(M1.all_volumes, all_centroids, box_dual_1, False)
...
#
#
#k=fit[2]-(lg[-1]-lg[0])/len(lg)
#a1=fit[0]
#b1=fit[1]
#
#ind_inferior=int((-2*b1-(4*b1**2-4*k*3*a1*s)**0.5)/(6*a1))
#ind_superior=int((-2*b1+(4*b1**2-4*k*3*a1*s)**0.5)/(6*a1))
...
#OP3[ID_global1,IDs_AMS1]=1
#nivel_0_v=M1.mb.get_entities_by_type_and_tag(vm, types.MBHEX, np.array([L3_ID_tag]), np.array([1]))
...
vm=M1.mb.create_meshset()
M1.mb.add_entities(vm,vertices)
v_nivel_0=M1.mb.get_entities_by_type_and_tag(vm, types.MBHEX, np.array([L3_ID_tag]), np.array([3]))
